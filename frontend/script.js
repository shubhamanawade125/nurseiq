...
function formatPatientComms(comms) {
    if (!comms) return '';
    let html = `<hr style="margin: 12px 0; border-color: #ccc;">`;
    html += `<b>📋 Patient Communication</b><br><br>`;
    if (comms.discharge_summary) html += `<b>Discharge Summary:</b><br>${comms.discharge_summary}<br><br>`;
    if (comms.medication_schedule) html += `<b>Medication Schedule:</b><br>${comms.medication_schedule}<br><br>`;
    if (comms.follow_up_instructions) html += `<b>Follow-up:</b><br>${comms.follow_up_instructions}`;
    return html;
}
...
sendBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        // Handle nested soapNote object OR flat fields
...
if (startRecBtn && stopRecBtn && recordingIndicator) {
    ...
    // Buttons not found in HTML — log which ones are missing
